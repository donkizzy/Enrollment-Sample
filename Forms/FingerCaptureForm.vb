...
Namespace Forms
	Partial Public Class FingerCaptureForm
		...
		Public Sub New()
			...
			AddHandler tool.SegmentManipulationEnded, AddressOf OnSegmentManipulationEndedAsync
...
#Region "Private form events"
		...
		Private Async Sub CaptureFormLoadAsync(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
			If _biometricClient IsNot Nothing AndAlso _subject IsNot Nothing Then
				...
				If (Not (Await NextTaskAsync())) Then
		...
		Private Async Sub BtnPreviousClickAsync(ByVal sender As Object, ByVal e As EventArgs) Handles btnPrevious.Click
			Dim index As Integer = _captureList.IndexOf(_current) - 1
			If index >= 0 Then
				Await StartTaskAsync(_captureList(index))
			End If
		End Sub

		Private Async Sub BtnNextClickAsync(ByVal sender As Object, ByVal e As EventArgs) Handles btnNext.Click
			Await NextTaskAsync()
		End Sub

		Private Async Sub BtnRescanClickAsync(ByVal sender As Object, ByVal e As EventArgs) Handles btnRescan.Click
			_current.Image = Nothing
			Dim index As Integer = _captureList.IndexOf(_current)
			lvQueue.Items(index).ForeColor = Color.Black

			Await StartTaskAsync(_current)
		End Sub

		Private Async Sub BtnAcceptClickAsync(ByVal sender As Object, ByVal e As EventArgs) Handles btnAccept.Click
			EnableControls()
			If _current IsNot Nothing Then
				...
				Try
					Dim performedTask = Await _biometricClient.PerformTaskAsync(task)
					Await OnCreateTemplateCompletedAsync()
		...
		Private Async Sub LvQueueSelectedIndexChangedAsync(ByVal sender As Object, ByVal e As EventArgs) Handles lvQueue.SelectedIndexChanged
		...
		Private Async Function OnCreateTemplateCompletedAsync() As Tasks.Task
		...
		Private Async Sub OnSegmentManipulationEndedAsync(ByVal sender As Object, ByVal e As EventArgs)
		...
		Private Sub OnCaptureCompleted()
			Try
...
#Region "Private methods"
		...
		Private Async Function NextTaskAsync() As Tasks.Task(Of Boolean)
		...
		Private Async Function StartTaskAsync(ByVal task As NFinger) As Tasks.Task
			If _current IsNot Nothing Then
				If IsCaptureDone(_current) AndAlso (Not IsCreateTemplateDone(_current)) Then
			...
			If (Not IsCreateTemplateDone(_current)) Then
				...
				_isCapturing = True
				Try
					Dim performedTask = Await _biometricClient.PerformTaskAsync(biometricTask)
					OnCaptureCompleted()
				Catch ex As Exception
					SetError(ex.Message)
					Utilities.ShowError(ex)
